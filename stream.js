...
ffmpeg(process.env.RTSP_URL)
  ...
  .outputOptions([
    "-vf",
    "scale=640:360", // Resize to 640x360 (low quality)
    "-b:v",
    "500k", // Lower video bitrate
    "-r",
    "15", // Reduce frame rate to 15 FPS
    "-c:v",
    "libx264", // Use H.264 codec
    "-preset",
    "ultrafast", // Low latency processing
    "-crf",
    "30", // Higher CRF = lower quality
    "-c:a",
    "aac", // Use AAC audio codec
    "-b:a",
    "64k", // Lower audio bitrate
    "-hls_time",
    "2", // Shorter segment duration (2s for real-time playback)
    "-hls_list_size",
    "6", // Keep only last 6 segments
    "-hls_flags",
    "delete_segments+append_list", // Delete old segments
    "-f",
    "hls", // Force format as HLS
  ])
  .output(outputHLS)
